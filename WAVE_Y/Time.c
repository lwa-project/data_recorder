...
/*
inline size_t    getMicrosPastMidnight(){
	struct timeval dt;
	gettimeofday(&dt, NULL);
	size_t s0 = (size_t) dt.tv_sec;
	size_t sd = SEC_PER_DAY;
	size_t s  = dt.tv_sec - ((s0 / sd) * sd);
	size_t rv = (s * 1000000l) + dt.tv_usec;
	return rv;
}
*/

inline size_t getMicrosSinceEpoch(size_t tt){
	if (tt == 0){
		struct timeval dt;
		gettimeofday(&dt, NULL);
		size_t rv = (((size_t) dt.tv_sec) * 1000000l) + dt.tv_usec;
		return rv;
	} else {
		size_t rv = tt/196l;
		return rv;
...
TimeStamp getTimestampFromUSPE(size_t uspe){
	TimeStamp rv;
	struct timeval dt;
	dt.tv_sec  = uspe / 1000000;
	dt.tv_usec = uspe - (dt.tv_sec * 1000000);
...
char* HumanTimeFromTag(size_t tt){
	size_t uspe = getMicrosSinceEpoch(tt);
	TimeStamp ts=getTimestampFromUSPE(uspe);
...
size_t getElapsedMilliseconds(TimeStamp start, TimeStamp stop){
	return (((stop.MJD*1000l*60l*60l*24l)+(stop.MPM))-	((start.MJD*1000l*60l*60l*24l)+(start.MPM)));
}
