...
size_t FileSystem_GetFreeSpace(FileSystem * fs){
	...
	size_t file_bytes=0;
	size_t f_bytes= sizeof(FileSystemBlock);
	while(index<=fs->fileSystemHeaderData->filesystemInfo.number_files_present){
		file_bytes+=fs->fileSystemHeaderData->fileInfo[index].size;
		f_bytes+=(fs->fileSystemHeaderData->fileInfo[index].stopPosition - fs->fileSystemHeaderData->fileInfo[index].startPosition)-fs->fileSystemHeaderData->fileInfo[index].size ;
		index++;
	}
...
size_t FileSystem_GetFreeSpaceContiguous(FileSystem * fs){
	assert(fs!=NULL);
	assert(fs->rawDeviceHandle!=-1);
	assert(fs->IMFS_Size!=0);
	assert(fs->IMFS_ptr!=NULL);
	if (fs->fileSystemHeaderData->filesystemInfo.number_files_present == 0){
		return fs->fileSystemHeaderData->filesystemInfo.diskSize;
	}
	size_t numFiles     = fs->fileSystemHeaderData->filesystemInfo.number_files_present;
	size_t diskSize     = fs->fileSystemHeaderData->filesystemInfo.diskSize;
	size_t dataStart    = sizeof(FileSystemBlock);
	size_t largestSoFar = 0;
	int c_idx           = 0;
	int n_idx           = c_idx + 1;

	while(c_idx <= numFiles){
		size_t start = (c_idx < 1       )  ? (dataStart) : (fs->fileSystemHeaderData->fileInfo[c_idx].stopPosition);
		size_t stop  = (n_idx > numFiles)  ? (diskSize)  : (fs->fileSystemHeaderData->fileInfo[n_idx].startPosition);
		size_t freespace = stop - start;
		if (freespace >largestSoFar){
			largestSoFar = freespace;
		}
		c_idx++;
		n_idx = c_idx + 1;
	}
...
StatusCode FileSystem_SetVolumeName(FileSystem* fs, const char* name){
	int slen = strlen(name);
	if (slen >= FILE_MAX_NAME_LENGTH){
		return FAILURE;
	} else {
		strncpy(fs->fileSystemHeaderData->filesystemInfo.volname,name,FILE_MAX_NAME_LENGTH+1);
		/*
		StatusCode _FileSystem_SynchronousWrite(
				FileSystem * fs,
				void* buf,
				off_t position,
				size_t count,
		...
		*/
		ssize_t bytesWritten=0;
		StatusCode sc = _FileSystem_SynchronousWrite(fs, fs->IMFS_ptr,0,sizeof(FileSystemInfo), &bytesWritten);
		if (bytesWritten!=sizeof(FileSystemInfo)){
			Log_Add("[FILE SYSTEM] Error: can't synch file system after volume name change.");
			return FAILURE;
		} else {
			return sc;
		}
