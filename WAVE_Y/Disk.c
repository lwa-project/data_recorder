...
StatusCode readBarcde(const char* deviceName, char * dest){
	char barcode[MAX_BARCODE_LENGTH];
	size_t ofs = offsetof(struct __FileSystemInfo, volname);
	size_t maxlen = MAX_BARCODE_LENGTH;
	int fd = open(deviceName, O_RDONLY);
	if (fd==-1)		return FAILURE;
	ssize_t result = pread(fd, barcode, maxlen, ofs);
	close(fd);
	if (result == maxlen){
		int len = strnlen(barcode,MAX_BARCODE_LENGTH);
		if (len == MAX_BARCODE_LENGTH){
			// volname is unterminated, and so neither a barcode nor a valid volume name

			return FAILURE;
		} else {
			if (!strcmp(barcode, DEFAULT_VOLUME_NAME)){
				// no barcode present
				dest[0]='\0'; // set dest to be an empty string
				return SUCCESS;
			} else {
				// barcode present
				strncpy(dest,barcode,MAX_BARCODE_LENGTH-1);
				return SUCCESS;
			}
		}
	} else {
...
/*
	...
	// pass 7: read barcodes of DRSUs and verify serial numbers are still valid
	for (i=0;i<devicesCount; i++){
		blocksUsed=0;
		switch(devices[i].type){
			case DRIVETYPE_UNKNOWN					: //fall through
			...
			case DRIVETYPE_INTERNAL_BLOCK_DEVICE	: //fall through
			case DRIVETYPE_EXTERNAL_BLOCK_DEVICE	: //fall through
			case DRIVETYPE_EXTERNAL_MULTIDISK_DEVICE: //fall through
			case DRIVETYPE_EXTERNAL_PARTITION		: //fall through
			default :
				sprintf(devices[i].barcode,"No Barcode");
				break;
			case DRIVETYPE_INTERNAL_MULTIDISK_DEVICE:
				if (readBarcde(devices[i].deviceName, devices[i].barcode) != SUCCESS){
						//offsetof(FileSystemInfo,volname)
						//sprintf(cmdTemp,"cat /LWA/runtime/barcodes | grep %s | head -n 1 | sed 's/^ *//' | tr -d \"\\n\"",&devices[i].deviceName[5]);
						//if (issueShellCommand(cmdTemp, devices[i].barcode, MAX_BARCODE_LENGTH) == -1) {
						return FAILURE;
				}
				break;
		}
	...
	// pass 8: print discovered drives
	for (i=0; i<devicesCount; i++){
