...
class Gnuplot
{
    ...
    public:
        ...
        Gnuplot(bool persist, const std::string &style = "points");

        /// plot a single std::vector at one go
        Gnuplot(bool persist, const std::vector<double> &x,
    ...
    ///   from data
    template<typename X, typename Y>
    Gnuplot& plot_xy_many(const std::vector<X>* x, const std::vector<Y>* y, const int i, const std::string *titles = NULL);
    ...
    Gnuplot& plot_imagef(const float * ucPicBuf,
							const double xmin,
							const double xmax,
							const int iWidth,
							const double ymin,
    ...
    Gnuplot& plot_imaged(const double * ucPicBuf,
    							 const double xmin,
    							 const double xmax,
    		                     const size_t iWidth,
    		                     const double ymin,
...
//
inline Gnuplot::Gnuplot(bool persist, const std::string &style)
...
//
inline Gnuplot::Gnuplot(bool persist, const std::vector<double> &x,
                 const std::string &title,
...
///   from data
template<typename X, typename Y>
Gnuplot& Gnuplot::plot_xy_many(const std::vector<X>* x, const std::vector<Y>* y, const int count, const std::string *titles){
	size_t sz = x[0].size();
	for(int i=0; i<count; i++){
		if ((x[i].size()!=sz) || (y[i].size()!=sz)){
			 throw GnuplotException("Length of the std::vectors differs");
			 return *this;
		}
	}
	...
	std::string name = create_tmpfile(tmp);
	if (name == ""){
        throw GnuplotException("Can't create temporary file.");
        return *this;
    }

	for (unsigned int sample = 0; sample < sz; sample++){
		for (int i = 0; i < count; i++){
			tmp << x[i][sample] << " " << y[i][sample] << " ";
		}
		tmp << std::endl;
	}
	tmp.flush();
...
{
    cmd("set output");
    cmd("set terminal " + Gnuplot::terminal_std  + " size 800,800");
...
//
Gnuplot& Gnuplot::plot_imagef(const float * ucPicBuf,
							 const double xmin,
							 const double xmax,
		                     const int iWidth,
		                     const double ymin,
                             ...
                             const std::string &title)
{
    ...
    if (name == "")
        return *this;
    std::cout << "tmpfilename = " << name << std::endl;
    //
    // write the data to file
    //
    int iIndex = 0;
    double xstep = (xmax - xmin)/(double)iWidth;
    double ystep = (ymax - ymin)/(double)iHeight;

    for(int iRow = 0; iRow < iHeight; iRow++)
    {
        for(int iColumn = 0; iColumn < iWidth; iColumn++)
        {
        	tmp << ((iColumn*xstep)+xmin) << " " << ((iRow*ystep)+ymin) << " " << ucPicBuf[iIndex++] << std::endl;
        }
    }
...
Gnuplot& Gnuplot::plot_imaged(const double * ucPicBuf,
							 const double xmin,
							 const double xmax,
		                     const size_t iWidth,
		                     const double ymin,
...
//
void Gnuplot::init(bool persist)
{
