...
typedef struct __MyFile{
	union {
		struct {
			...
			int 			newFile;			// flag indicating file was opened by create method
			size_t			sizeAtOpen;			// size of the file when opened, will be 0 for created files
			size_t			maxWrittenAddress;   // address of the highest byte written (if (mWA+1) <= sizeAtOpen, no change, otherwise grow file)
...
#define HLY(a)(COLOR_YELLOW a COLOR_NORMAL)
