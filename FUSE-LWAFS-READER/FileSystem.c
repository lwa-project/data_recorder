...
void _FileSystem_RegisterActiveFilesystem(FileSystem * fs){
	printf("[FILE SYSTEM] Registering Active File System %d:<%lu>\n",NumActiveFilesystems ,(size_t)fs);
...
void _FileSystem_UnregisterActiveFilesystem(FileSystem * fs){
	...
	while (i<NumActiveFilesystems){
		if (ActiveFileSystems[i]==fs){
			printf("[FILE SYSTEM] Unregistering Active File System %d:<%lu>\n",i ,(size_t)fs);
			for (j=i;j<(NumActiveFilesystems-1);j++){
				printf("[FILE SYSTEM] Copying record %d to %d\n",j+1,j);
...
void _FiLeSystem_CloseAllActiveSystems(){
	while (NumActiveFilesystems>0){
		printf("[FILE SYSTEM] FileSystem_Close(ActiveFileSystems[0]);\n");
...
boolean FileSystem_TestDiskForLWAFS(Disk * disk){
	...
	if (fs->rawDeviceHandle == -1) {
		printf("[FILE SYSTEM] TestDiskForLWAFS: Warning: could not open disk to test for LWAFS\n");
		return 0;
	}else{
		fs->diskInfo=disk;
		fs->IMFS_Size = sizeof(FileSystemBlock);
		if (_FileSystem_AllocateIMFS(fs)==FAILURE){
			printf("[FILE SYSTEM] TestDiskForLWAFS: Warning: Allocation failure\n");
		...
		if ((_FileSystem_SynchronousRead(fs, fs->IMFS_ptr,0,fs->IMFS_Size, &bytesRead)==FAILURE) ||	(bytesRead!=fs->IMFS_Size)){ // read the whole filesystem in....
			printf("[FILE SYSTEM] TestDiskForLWAFS: Warning: FileSystemSynchronousRead failure\n");
...
StatusCode FileSystem_Create(Disk * disk){
	...
	if (_FileSystem_AllocateIMFS(&fs)!=SUCCESS){
		printf("[FILE SYSTEM] Error: could not allocate IMFS\n");
		return FAILURE;
	}
	...
	if (fs.rawDeviceHandle == -1) {
		printf("[FILE SYSTEM] Error: could not open device\n");
		perror("open(...)");
		return FAILURE;
	}
	...
	printf("[FILE SYSTEM] WARNING: Creating filesystem on device %s !!!!\n",disk->deviceName);
	...
	if ((_FileSystem_SynchronousWrite(&fs, fs.IMFS_ptr, 0, fs.IMFS_Size, &bytesWritten)!=SUCCESS) || bytesWritten!=fs.IMFS_Size){
		printf("[FILE SYSTEM] FATAL error: couldn't write filesystem info to disk. data may be lost.\n");
		close(fs.rawDeviceHandle);
...
StatusCode FileSystem_Close(FileSystem * fs){
	...
	printf("[FILE SYSTEM] Closing filesystem.\n");
	if (fs->rawDeviceHandle!=-1){
		ssize_t bytesWritten;
...
StatusCode FileSystem_Verify(FileSystem * fs){
	printf("[FILE SYSTEM] Checking file system integrity...\n");
	size_t i=0;
	size_t foundcount=0;
	...
	if ((patternBuff==NULL)||(patternBuff==MAP_FAILED)){
		printf("[FILE SYSTEM] Error: can't allocate space for filesystem verification.\n");
		perror("mmap(...)");
		close(fs->rawDeviceHandle);
		exit(EXIT_CODE_FAIL);
	}
	if (!SilentVerify) printf("[FILE SYSTEM] Header says there are %lu out of %lu possible files.\n",fs->fileSystemHeaderData->filesystemInfo.number_files_present,fs->fileSystemHeaderData->filesystemInfo.maxFiles);
	for(i=1;i<fs->fileSystemHeaderData->filesystemInfo.maxFiles;i++){
		if (!SilentVerify) printf("[FILE SYSTEM] Checking index %.8lu of %.8lu\r\n",i,fs->fileSystemHeaderData->filesystemInfo.maxFiles);
		...
		if (fs->fileSystemHeaderData->fileInfo[i].startPosition!=0){
			...
			if (fs->fileSystemHeaderData->fileInfo[i].startPosition<fs->IMFS_Size){
				...
				printf("[FILE SYSTEM] File consistency check failed: Start position overlaps filesystem data.\n");
				printf("[FILE SYSTEM] Deleting File record.\n");
			...
			if (fs->fileSystemHeaderData->fileInfo[i].stopPosition < (fs->fileSystemHeaderData->fileInfo[i].startPosition+(2*fs->fileSystemHeaderData->filesystemInfo.chunkSize))){
				_FileSystem_DumpFileInfo(fs,i);
				printf("[FILE SYSTEM] File consistency check failed: End position is less than the minimum.\n");
				printf("[FILE SYSTEM] Deleting File record.\n");
				memset((void*)&fs->fileSystemHeaderData->fileInfo[i],0,sizeof(FileInfo));
				continue;
			}
			...
			if (fs->fileSystemHeaderData->fileInfo[i].size > (allocatedSize-2*fs->fileSystemHeaderData->filesystemInfo.chunkSize)){
				_FileSystem_DumpFileInfo(fs,i);
				printf("[FILE SYSTEM] File consistency check failed: Length exceeds allocated space.\n");
				printf("[FILE SYSTEM] Truncating file to fit allocated space.\n");
			...
			if (allocatedSize>(fs->fileSystemHeaderData->filesystemInfo.diskSize-fs->IMFS_Size)){
				_FileSystem_DumpFileInfo(fs,i);
				printf("[FILE SYSTEM] File consistency check failed: Length exceeds volume size.\n");
				printf("[FILE SYSTEM] Deleting File record.\n");
				memset((void*)&fs->fileSystemHeaderData->fileInfo[i],0,sizeof(FileInfo));
				continue;
			}
			...
			if (s!=SUCCESS || bytesRead!=fs->fileSystemHeaderData->filesystemInfo.chunkSize){
				printf("[FILE SYSTEM] File consistency check failed: Can't read start tag.\n");
				continue;
			}
			if (!_FileSystem_CheckPattern(patternBuff, fs->fileSystemHeaderData->filesystemInfo.chunkSize)){
				_FileSystem_DumpFileInfo(fs,i);
				printf("[FILE SYSTEM] File consistency check failed: File start tag missing or mismatched.\n");
				printf("[FILE SYSTEM] Deleting File record.\n");
				memset((void*)&fs->fileSystemHeaderData->fileInfo[i],0,sizeof(FileInfo));
				continue;
			}
			...
			if (s!=SUCCESS || bytesRead!=fs->fileSystemHeaderData->filesystemInfo.chunkSize){
				printf("[FILE SYSTEM] File consistency check failed: Can't read stop tag.\n");
				continue;
			}
			if (!_FileSystem_CheckPattern(patternBuff, fs->fileSystemHeaderData->filesystemInfo.chunkSize)){
				_FileSystem_DumpFileInfo(fs,i);
				printf("[FILE SYSTEM] File consistency check failed: File stop tag missing or mismatched.\n");
				printf("[FILE SYSTEM] Deleting File record.\n");
				memset((void*)&fs->fileSystemHeaderData->fileInfo[i],0,sizeof(FileInfo));
				continue;
			}
			...
			for (j=1;j<i;j++){
				if (_FileSystem_RegionOverlaps(fs->fileSystemHeaderData->fileInfo[j].startPosition, fs->fileSystemHeaderData->fileInfo[j].stopPosition-1, fs->fileSystemHeaderData->fileInfo[i].startPosition, fs->fileSystemHeaderData->fileInfo[i].stopPosition-1 )){
					_FileSystem_DumpFileInfo(fs,i);
					printf("[FILE SYSTEM] File consistency check failed: file boundaries overlap.\n");
					printf("[FILE SYSTEM] Conflicting File:\n");
					_FileSystem_DumpFileInfo(fs,j);
					printf("[FILE SYSTEM] Deleting second file record.\n");
					memset((void*)&fs->fileSystemHeaderData->fileInfo[i],0,sizeof(FileInfo));
					continue;
				}
			}
			...
			if (fs->fileSystemHeaderData->fileInfo[i].isOpen){
				_FileSystem_DumpFileInfo(fs,i);
				printf("[FILE SYSTEM] File consistency check failed: File was not closed properly. Perhaps the operation was aborted?\n");
				fs->fileSystemHeaderData->fileInfo[i].isOpen=false;
				// no action necessary, and we should keep going. this is just to notify user ...
			}
			if (!SilentVerify) printf("[FILE SYSTEM] Found file: '%s'\n",fs->fileSystemHeaderData->fileInfo[i].name);
		}
	}
	if (foundcount!=fs->fileSystemHeaderData->filesystemInfo.number_files_present){
		printf("[FILE SYSTEM] Filesystem consistency check failed: number of files reported does not match the number of files found.\n");
	...
	printf("[FILE SYSTEM] File check complete.\n");
	return SUCCESS;
}
...
StatusCode FileSystem_Open(FileSystem* fs, Disk * disk){
	...
	*/
	fs->rawDeviceHandle = open(disk->deviceName, O_RDWR | O_DIRECT );
	if (fs->rawDeviceHandle == -1) {
		printf("[FILE SYSTEM] Error: could not open filesystem\n");
		...
		if ((buf!=NULL)&&(buf!=MAP_FAILED)){
			...
			if ((_FileSystem_SynchronousRead(fs, buf,0,sizeof(FileSystemInfo),&bytesRead)==FAILURE) ||
				...
				printf("[FILE SYSTEM] Error: could not read file system info header\n");

			}
			fs->IMFS_Size = sizeof(FileSystemBlock);
			//printf("----- imfs size = %lu\n",fs->IMFS_Size);
			printf("[FILE SYSTEM] Detected a filesystem size of %lu.\n",fs->IMFS_Size);
			...
			if (fs->IMFS_Size > (512l*1024l*1024l)){
				printf("[FILE SYSTEM] Error: filesystem reports too large a size, max is 512 MiB, or about 128,000 files\n");
				exit(EXIT_CODE_FAIL);
			}
			if (fs->IMFS_Size < (32l*4096l)){
				printf("[FILE SYSTEM] Error: filesystem reports too small of a size, min is 128 kiB, or 32 files\n");
				exit(EXIT_CODE_FAIL);
			}

			if (_FileSystem_AllocateIMFS(fs)==FAILURE){
				printf("[FILE SYSTEM] Allocation failure\n");
				return FAILURE;
			}
			...
			if ((_FileSystem_SynchronousRead(fs, fs->IMFS_ptr,0,fs->IMFS_Size, &bytesRead)==FAILURE) ||
				...
				printf("[FILE SYSTEM] FileSystemSynchronousRead failure\n");
				return FAILURE;
			}
			fs->fileSystemHeaderData=(FileSystemBlock *)fs->IMFS_ptr;
			printf( "                   [FILE SYSTEM] %s: "
			...
			if (FileSystem_Verify(fs)==FAILURE){
				printf("[FILE SYSTEM] FileSystemVerify failure\n");
				return FAILURE;
			}
			...
			if ((_FileSystem_SynchronousWrite(fs,fs->IMFS_ptr,0,fs->IMFS_Size, &bytesWritten)==FAILURE) ||
				...
				printf("[FILE SYSTEM] FileSystemSynchronousWrite failure\n");
				return FAILURE;
			}
			...
			printf("[FILE SYSTEM] Error: could not allocate space for filesystem header\n");
...
StatusCode FileSystem_ListFiles(FileSystem * fs){
	...
	printf("[FILE SYSTEM] Directory listing for volume '%s'\n",fs->fileSystemHeaderData->filesystemInfo.volname);
	while(index<fs->fileSystemHeaderData->filesystemInfo.number_files_present+1){
		sprintf(buf2,"%.2lu/%.2lu/%.4lu %.2lu:%.2lu:%.2lu",
				...
				fs->fileSystemHeaderData->fileInfo[index].creationDate.minute,
				fs->fileSystemHeaderData->fileInfo[index].creationDate.second);
		printf("[FILE SYSTEM] \t%.1s %.20s  %10lu %s\n",
...
size_t _FileSystem_RangeConflictsIgnore(FileSystem * fs, size_t start, size_t stop, size_t ignoreindex){
	size_t i=0;
	size_t startf,stopf;
	//printf("Trying range %lu to %lu\n",start,stop);
	for(i=1;i<fs->fileSystemHeaderData->filesystemInfo.number_files_present+1;i++){
...
StatusCode FileSystem_CreateFile(FileSystem * fs, char* name, size_t size, boolean overwrite, int* fileIndex){
	...
	*fileIndex = -1;
	...
	if (idx!=-1){
		if (! overwrite){
			//printf("[FILE SYSTEM] Can't create file, file with name '%s' already exists\n",name);
			*fileIndex = -1;
			return FAILURE;
		} else {
	...
	//printf("[FILE SYSTEM] Creating file '%s' of size %lu bytes\n",name,size);
	FileInfo fi;
	...
	size_t index=fs->fileSystemHeaderData->filesystemInfo.number_files_present+1; // +1 is because the 0th file is actually the filesystem header
	if (index>=fs->fileSystemHeaderData->filesystemInfo.maxFiles){
		printf("[FILE SYSTEM] Error could not create file, insufficient record space.\n");
		*fileIndex = -1;
		return FAILURE;
	}
	...
	if (noconflicts && ((fi.startPosition<fs->fileSystemHeaderData->filesystemInfo.diskSize) &&
		...
		void * patternBuff=mmap(
				...
				0);
		if ((patternBuff==NULL)||(patternBuff==MAP_FAILED)){
			printf("[FILE SYSTEM] Error: can't allocate memory for file creation.\n");
			perror("[FILE SYSTEM] mmap(...)");
			*fileIndex = -1;
			return FAILURE;
		}
		...
		if(	_FileSystem_SynchronousWrite(	fs,patternBuff,fi.startPosition,fs->fileSystemHeaderData->filesystemInfo.chunkSize,&bytesWritten) == FAILURE ||
			...
			printf("[FILE SYSTEM] Error: can't write file start tag.\n");
			perror("[FILE SYSTEM] mmap(...)");
			*fileIndex = -1;
			return FAILURE;
		}
		...
		if(	_FileSystem_SynchronousWrite(fs, patternBuff, fi.stopPosition-fs->fileSystemHeaderData->filesystemInfo.chunkSize,fs->fileSystemHeaderData->filesystemInfo.chunkSize, &bytesWritten)==FAILURE ||
				bytesWritten !=fs->fileSystemHeaderData->filesystemInfo.chunkSize){
			printf("[FILE SYSTEM] Error: can't write file stop tag.\n");
			perror("[FILE SYSTEM] mmap(...)");
			*fileIndex = -1;
			return FAILURE;
		}
		...
		if (_FileSystem_SynchronousWrite(fs,fs->IMFS_ptr,0,fs->IMFS_Size, &bytesWritten)==FAILURE ||
				...
				printf("[FILE SYSTEM] Error: can't synch file system after create file.\n");
				*fileIndex = -1;
				return FAILURE;
		}
		//printf("[FILE SYSTEM] File created successfully\n");
		*fileIndex = index;

		return SUCCESS;
	} else {
		printf("[FILE SYSTEM] Error could not create file, insufficient contiguous drive space.\n");
		*fileIndex = -1;
		return FAILURE;
	}
}
...
StatusCode FileSystem_OpenFile(FileSystem * fs, MyFile* fileptr,const char* fileName, FILEMODE mode){
	...
	if (idx==-1){
		printf("[FILE SYSTEM] File not found: '%s'\n",fileName);
		fileptr->index = -1;
		return FAILURE;
	} else {
		//printf("INDEX############################################### %d\n",idx);
		if (fs->fileSystemHeaderData->fileInfo[idx].isOpen){
			printf("[FILE SYSTEM] File already open: '%s'\n",fileName);
			return FAILURE;
		}
		fileptr->index=-1;
		//printf(HLR("INDEX = %d\n"),idx);
		...
		if (bytesWritten!=fs->IMFS_Size || s!=SUCCESS){
			printf("[FILE SYSTEM] Error: can't synch file system after open file.\n");
			return FAILURE;
		} else {
			return s;
		}
...
StatusCode FileSystem_CloseFile(MyFile* file){
	//fprintf(stderr,HLR("\n\nCLOSE: '%s'\n\n"),file->fs->fileSystemHeaderData->fileInfo[file->index].name);
	...
	if (bytesWritten!=file->fs->IMFS_Size){
		printf("[FILE SYSTEM] Error: can't synch file system after close file.\n");
		return FAILURE;
	} else {
		return s;
	}
...
StatusCode FileSystem_DeleteFile(FileSystem * fs, int fileIndex){
	...
	if (fileIndex<1 || (fileIndex > fs->fileSystemHeaderData->filesystemInfo.number_files_present)){
		printf("[FILE SYSTEM] File not deleted: invalid index specified. '%d'\n",fileIndex);
		return FAILURE;
	}
	...
	if ((bytesWritten != fs->IMFS_Size) || (s != SUCCESS)){
		printf("[FILE SYSTEM] Error: could not synch IMFS after deleting file\n");
		return FAILURE;
	}
	return SUCCESS;
}
	...
	if (file->readOpInfo.aio_fildes==-1) {
		...
		if ((file->fs->fileSystemHeaderData->fileInfo[file->index].currentPosition+bytes)>file->fs->fileSystemHeaderData->fileInfo[file->index].size){// case for the last packet to be read
			if(!file->lastRPacket){
				...
				printf("[FILE SYSTEM] Error: tried to read a last packet more than once.\n");
				printf("[FILE SYSTEM] Cpos %15lu\n",file->fs->fileSystemHeaderData->fileInfo[file->index].currentPosition);
				printf("[FILE SYSTEM] Size %15lu\n",file->fs->fileSystemHeaderData->fileInfo[file->index].size);
				printf("[FILE SYSTEM] breq %15lu\n",bytes);
				exit(EXIT_CODE_FAIL);
			}
		} else{
		...
		if (file->readOpInfo.aio_offset+bytes > (file->fs->fileSystemHeaderData->fileInfo[file->index].stopPosition - file->fs->fileSystemHeaderData->filesystemInfo.chunkSize)){
			printf("[FILE SYSTEM] Error: Attempt to read past the end of the file.\n");
			...
			if (status<0) {
				bzero( (char *)&file->readOpInfo, sizeof(struct aiocb) );
				file->readOpInfo.aio_fildes=-1;
				printf("[FILE SYSTEM] Error: Couldn't start asynchronous read.\n");
				perror("aio_read(...)");
				return NOT_READY;
			} else {
				return SUCCESS;
		...
		printf("[FILE SYSTEM] Error: Tried to start a read when a read was already in progress.\n");
		return FAILURE;
	}
}
...
StatusCode FileSystem_GetAsynchronousBytesRead(MyFile* file, size_t *result){
	...
	// check for errors
	if (aioResult<0){
		printf("[FILE SYSTEM] Error: Asynchronous read reported an error.\n");
		...
		*result=0;
		...
		if (!file->lastRPacket){
			if ((size_t)aioResult != file->readOpInfo.aio_nbytes){
				printf("[FILE SYSTEM] Error: Asynchronous read failed to read all bytes.\n");
			}
			bzero( (char *)&file->readOpInfo, sizeof(struct aiocb) );
			file->readOpInfo.aio_fildes=-1;
	...
	if (file->writeOpInfo.aio_fildes==-1) {
		...
		if (bytes<file->fs->fileSystemHeaderData->filesystemInfo.chunkSize){// case for the last packet written
			if(!file->lastWPacket){
				...
				printf("[FILE SYSTEM] Error: tried to write a last packet more than once.\n");
				exit(EXIT_CODE_FAIL);
			}
		} else{
		...
		if (file->writeOpInfo.aio_offset + bytes > (file->fs->fileSystemHeaderData->fileInfo[file->index].stopPosition - file->fs->fileSystemHeaderData->filesystemInfo.chunkSize)){
			//printf("Error: Attempt to write past the end of the file.\n");
			/*printf("\tFile Start: %lu\n\tFile Stop: %lu\n\tCurrent Position: %lu\n\tBytes Tried to Write: %lu\n",
					file->fs->fileSystemHeaderData->fileInfo[file->index].startPosition,
			...
			if (status<0) {
				bzero( (char *)&(file->writeOpInfo), sizeof(struct aiocb) );
				file->writeOpInfo.aio_fildes=-1;
				printf("[FILE SYSTEM] Error: Couldn't start asynchronous write.\n");
				perror("aio_write(...)");
				return FAILURE;
			} else {
				return SUCCESS;
		...
		printf("[FILE SYSTEM] Error: Tried to start a write when a write was already in progress.\n");
		return FAILURE;
	}
}
...
StatusCode FileSystem_GetAsynchronousBytesWritten(MyFile* file, size_t* result){
	...
	// check for errors
	if (aioResult<0){
		printf("[FILE SYSTEM] Error: Asynchronous write reported an error. %ld\n", aioResult);
		perror("[FILE SYSTEM] aio_return(...)");
		bzero( (char *)&file->writeOpInfo, sizeof(struct aiocb) );
		file->writeOpInfo.aio_fildes=-1;
		*result =  0;
		...
		if (!file->lastWPacket){
			if ((size_t)aioResult != file->writeOpInfo.aio_nbytes){
				printf("[FILE SYSTEM] Error: Asynchronous write failed to write all bytes.\n");
			}
			bzero( (char *)&file->writeOpInfo, sizeof(struct aiocb) );
			file->writeOpInfo.aio_fildes=-1;
			file->fs->fileSystemHeaderData->fileInfo[file->index].currentPosition+=(size_t)aioResult;
...
void FileSystem_GetFileMetaData(FileSystem * fs, int index, FileMetaData** data){
	if (!fs || index<0 || index>fs->fileSystemHeaderData->filesystemInfo.number_files_present || !data){
		printf("[FILE SYSTEM] Error: tried to get metadata, but no filesystem or bad index or bad result pointer.\n");
		return;
	}
	...
	*data = &(fs->fileSystemHeaderData->fileInfo[index].metaData);
	printf("[FILE SYSTEM] Get File Metadata for '%s'.\n",fs->fileSystemHeaderData->fileInfo[index].name);
}
...
void FileSystem_SetFileMetaData(FileSystem * fs, int index, size_t StartMPM, size_t StartMJD, size_t reference, size_t StopMPM, size_t StopMJD, char * format){
	if (!fs || index<0 || index>fs->fileSystemHeaderData->filesystemInfo.number_files_present){
		printf("[FILE SYSTEM] Error: specified metadata, but no filesystem or bad index.\n");
		return;
	}
	...
	printf("[FILE SYSTEM] Set File Metadata for '%s'.\n",fs->fileSystemHeaderData->fileInfo[index].name);
}
void FileSystem_SetFileMetaDataIsComplete(FileSystem * fs, int index, int complete){
	...
	printf("[FILE SYSTEM] Set Complete Flag for '%s'.\n",fs->fileSystemHeaderData->fileInfo[index].name);
}
