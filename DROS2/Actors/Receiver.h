...
public:
	...
	bool setNewFmtDrx(int decFactor){
		...
		switch(decFactor){
			...
			case 49:  newFormat = DataFormat::getFormatByName("DRX_FILT_5"); return true; break;
	...
	void report(){
		...
		progressbar(pct, 40, ss);
		ss << ANSI::colorSpec(basic,white,black);
		LOG(L_INFO, ss.str());
		ss.clear();
		ss.str("");

		ss << "[Receiver] Receive rate: ";
		...
		ss << ANSI::colorSpec(basic,white,black);
		LOG(L_INFO, ss.str());
		LOGC(L_INFO, "[Receiver] Subscribers "+LXS(buf->getSubscriberCount()), basic,white,black);
	}
...
*/
			...
			// something received
				...
				// quick check the common case, all packets right size
