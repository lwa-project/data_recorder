...
double Drx8FrameGenerator::getFrequency(uint64_t fs, uint32_t freqCode){
	...
	start(TimeKeeper::getTT())
{
	...
	*/
	sig[0]=_sig0;
	sig[1]=_sig1;
	for (int j=0; j<DRX8_TUNINGS; j++){

		frames[j] = (Drx8Frame*) malloc(numFrames* sizeof(Drx8Frame));
		assert(frames[j]!=NULL);
		bzero((void*) frames[j], numFrames* sizeof(Drx8Frame));
		cout << "allocated " << (numFrames* sizeof(Drx8Frame)) << " bytes of storage (" << numFrames << " frames x " << sizeof(DrxFrame) << " bytes/frame.\n";
	}
	generate();
}
...
void Drx8FrameGenerator::__printFrame(Drx8Frame* f, bool compact, bool single){
	if (compact){
		...
		for(int i=0; i<1024; i++){
			printf("%05hx ",(int)f->samples[i].packed);
			if (((i & 0xf) == 0xf) || single){
				cout << endl;
			}
		}
...
void Drx8FrameGenerator::generate(){
	...
	for (int tuning=0; tuning<DRX8_TUNINGS; tuning++){
		for(size_t frm = 0; frm <numFrames; frm++){
			...
			if (bitPattern){
				for(size_t j=0; j<DRX8_SAMPLES_PER_FRAME; j++){
					frames[tuning][frm].samples[j].packed = (uint16_t) ((j+frm) & 0xffffll);
				}
			} else {
				switch(pol){
				case 0: /*X*/
					if (!correlatorTest){
						for(size_t j=0; j<DRX8_SAMPLES_PER_FRAME; j++){
							size_t sampleNumber=((frm >> 2) * DRX8_SAMPLES_PER_FRAME + j);
				...
				case 1: /*Y*/
					if (!correlatorTest){
						...
						memcpy((void*)&frames[tuning][frm].samples[0], (void*)&frames[tuning][ frm - 1 ].samples[0], DRX8_FRAME_SIZE);
					} else {
...
Drx8Frame * Drx8FrameGenerator::next(){
	...
	frames[cur_tuning][index].header.timeTag = __builtin_bswap64(((curFrame>>2) * DRX8_SAMPLES_PER_FRAME * ((uint64_t)decFactor))+start);

	DrxFrame * rv = &frames[cur_tuning][index];
	cur_pol++;
	if (cur_pol == DRX8_POLARIZATIONS){
		cur_pol = 0;
		cur_tuning++;
		if (cur_tuning == DRX8_TUNINGS){
			cur_tuning = 0;
			cur_index ++;
